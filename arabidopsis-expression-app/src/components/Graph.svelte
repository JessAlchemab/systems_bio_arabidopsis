<script lang="ts">
  ...
  import data100 from "../data/500_fold/500_fold_increase_final.json";
  ...
  function createInitialGraph(data: {
    ...
    cyInstance = cytoscape({
      ...
      layout: {
        name: "preset", // i dont want to calculate node positions on the go, it slows everything down too much.
      },
      style: [
        {
          selector: "node",
    ...
    cyInstance
      ...
      .forEach(
        (node: {
          ...
          if (node.data("name") === "AT1G24260") {
            console.log(node.data(selectedFileName));
          }
...
<h3>
  Genes showing significance through linear regression or a 500-fold increase
